...
class DemandModel:
    ...
    def __init__(self, city_name, demand_model_config,
        ...
        self.valid_zones = self.get_valid_zones(
            demand_model_config['valid_zones_thresh'])
    ...
    def get_neighbors_dicts(self):
        ...
        for i in self.grid_matrix.index:
            for j in self.grid_matrix.columns:
                ...
                if zone not in self.grid.index:
                    continue

                i_low = i - 1 if i - 1 >= 0 else 0
                i_up = i + 1 if i + 1 < len(self.grid_matrix.index) \
                    else len(self.grid_matrix.index) - 1

                j_low = j - 1 if j - 1 >= 0 else 0
                j_up = j + 1 if j + 1 < len(self.grid_matrix.columns) \
                    else len(self.grid_matrix.columns) - 1
                ...
                for ii in range(i - 1, i + 2):
                    for jj in range(j - 1, j + 2):
                        ...
                        if ii < i_low or ii > i_up \
                                ...
                                or self.grid_matrix.iloc[ii, jj] not in self.grid.index:
                            neighbors_dict[int(zone)] \
                                .update({iii: None})
                        else:
                            neighbors_dict[int(zone)] \
                                .update({iii: self.grid_matrix.iloc[ii, jj]})
    ...
    def save_results(self):
        demand_model_path = os.path.join(
            ...
            + str(self.bin_side_length),
        )

        os.makedirs(demand_model_path, exist_ok=True)

        # with open(os.path.join(demand_model_path,
        #                        "city_obj.pickle"), "wb") as f:
        #     pickle.dump(self, f)
        ...
        pd.DataFrame(self.neighbors_dict).fillna(np.nan).to_pickle(
            os.path.join(demand_model_path, "neighbors_dict.pickle")
        )
        pd.DataFrame(self.neighbors_dict).fillna(np.nan).to_csv(
