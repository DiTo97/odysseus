...
class ChargingPrimitives:
	...
	def charge_vehicle(
		...
		def check_queuing():
			"""
			Check whether workers queuing is enabled. If it is False, check whether the amount of
			charging requests doesn't exceed the amount of available workers.
			"""
	...
	def check_system_charge(self, booking_request, vehicle, charging_strategy):
		if charging_strategy == "reactive":
			...
			exit("Charging strategy is not supported")
