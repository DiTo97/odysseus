...
class SimOutput():
	...
	def __init__(self, sim):
		...
		if self.sim_general_conf["save_history"]:
			...
			self.sim_stats.loc["tot_charging_return_distance"] = sim.chargingStrategy.tot_charge_return_distance
			...
			if len(self.sim_charges) and "system" in self.sim_charges.operator.unique():
				...
				tot_charges_duration = self.sim_charges.duration.sum()

				self.sim_stats.loc["fraction_duration_system"] = \
					self.sim_charges.groupby("operator") \
						.duration.sum().loc["system"] \
					/ tot_charges_duration if tot_charges_duration > 0 else 0
			else:
